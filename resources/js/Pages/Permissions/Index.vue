...
watch(selectAll, (value) => {
    if (value && props.permissions?.data) {
...
watch(selectedPermissions, (value) => {
    if (props.permissions?.data) {
        selectAll.value = value.length === props.permissions.data.length && props.permissions.data.length > 0;
    }
}, { deep: true });
...
const openAssignModal = (permission) => {
    assignForm.value = {
...
const submitPermission = () => {
    if (modalMode.value === 'create') {
        router.post(route('permissions.store'), form.value, {
            ...
            onError: (errors) => {
                console.error('Create permission errors:', errors);
            }
        });
    } else {
        router.put(route('permissions.update', editingPermission.value.id), form.value, {
            onSuccess: () => {
                showPermissionModal.value = false;
            },
            onError: (errors) => {
                console.error('Update permission errors:', errors);
            }
        });
    }
};
...
const assignToRole = () => {
    router.post(route('permissions.assign-to-role', assignForm.value.permission_id), {
        ...
        onError: (errors) => {
            console.error('Assign to role errors:', errors);
        }
    });
};

const deletePermission = (permission) => {
    if (confirm('Are you sure you want to delete this permission? This may affect users and roles that have this permission.')) {
        router.delete(route('permissions.destroy', permission.id), {
            preserveScroll: true,
            onError: (errors) => {
                console.error('Delete permission errors:', errors);
            }
        });
    }
};
...
const bulkDelete = () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedPermissions.value.length} permissions?`)) {
        // You'll need to implement bulk delete for permissions
        alert('Bulk delete for permissions not implemented yet');
        selectedPermissions.value = [];
...
<template>
    ...
    <AppLayout>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white rounded-lg shadow mb-6">
                    <div class="p-4 flex flex-col sm:flex-row gap-4 justify-between">
                        ...
                        <!-- Bulk Actions -->
                        <div v-if="selectedPermissions.length > 0" class="flex items-center gap-3">
                            <span class="text-sm text-gray-600">{{ selectedPermissions.length }} selected</span>
                ...
                <div class="bg-white rounded-lg shadow overflow-hidden">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="permission in permissions.data" :key="permission.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-right text-sm font-medium">
                                    <button @click="openAssignModal(permission)" class="text-green-600 hover:text-green-900 mr-3" title="Assign to Role">
                                        <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                                        </svg>
                                    </button>
                                    <button @click="openEditModal(permission)" class="text-blue-600 hover:text-blue-900 mr-3" title="Edit Permission">
                                        <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </button>
                                    <button @click="deletePermission(permission)" class="text-red-600 hover:text-red-900" title="Delete Permission">
                                        <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Pagination -->
                    <div v-if="permissions && permissions.links" class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
                        <div class="flex items-center justify-between">
                            <div class="text-sm text-gray-700">
                                Showing <span class="font-medium">{{ permissions.from || 0 }}</span> to <span class="font-medium">{{ permissions.to || 0 }}</span> of <span class="font-medium">{{ permissions.total || 0 }}</span> results
                            </div>
        ...
        <div v-if="showPermissionModal" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                        ...
                        <form @submit.prevent="submitPermission" class="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    type="text"
                                    required
                                    placeholder="e.g., edit users, view reports"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                >
                                <p class="mt-1 text-xs text-gray-500">Use format: action-resource (e.g., view-users, create-posts)</p>
                            </div>
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <form @submit.prevent="assignToRole" class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Permission</label>
                                <input
                                    :value="assignForm.permission_name"
                                    type="text"
                                    disabled
